...
\begin{proof}
	...
	We conclude by \cref{The:BaireCategory}.
\end{proof}
...
\begin{proof}\
	\begin{enumerate}
		...
		Finite intersection is left for exercise.
...
\begin{theorem}[Nagata–Smirnov metrization theorem]\label{The:NagataSmirnov}
  A topological space $(X,\tau)$ is metrizable iff it is $T_4$ and has a $\sigma$-locally finite base, i.e., $\beta = \bigcup_{n=1}^\infty \beta_n$ with $\beta_n$ locally finite (i.e., for all $x \in X$, there exists a neighborhood $U$ of $x$ such that $U$ intersects finitely many elements of $\beta_n$).
\end{theorem}
\begin{proof}\
  \begin{itemize}
    ...
    Then we know $\bigcup_{B_x \in \beta_n} \overline{B_x} = \overline{\bigcup_{B_x \in \beta_n} B_x}$.
    Thus $C_n$ is closed.
...
\begin{proof}
  ...
  It suffices to show that every Cauchy sequence in $f(X)$ converges in $Y$ (by the fact that $f(X)$ is dense in $Y$).
  ...
  The proof is similar and left for exercise.
\end{proof}
...
\end{theorem}
\begin{proof}
  Suppose $\{U_n\}_{n \in \bbN}$ is a family of dense open sets (the finite case is left for exercise).
...
\begin{remark}\label{Rem:ContCompactNecInDini}
  ...
  Suppose $X=[0,1]$. Let $f_n(x) \defeq x^n$, then
  \[
  f(x) = \begin{dcases}
  0 & 0 \le x < 1 \\
  1 & x=1  
 ...
 \]
  is not continuous.
  Clearly, the convergence is not uniform.
  
  Suppose $X = \bbR$, which is not compact. Define
  \[
    f_n(x) \defeq \begin{dcases}
    1 & x \le n \\
    1+n-x & n <x \le n+1 \\
    0 & x > n+1  
 \end{dcases}.
  \]
  Then $f_n$ converges to $f(x) \equiv 1$. Clearly, the convergence is not uniform.
\end{remark}
...
\begin{proof}
  ...
  The idea is that $\bbQ$ is dense in $\bbR$. The rest of the proof is left for exercise.
\end{proof}
