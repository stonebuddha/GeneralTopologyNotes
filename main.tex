...
\begin{definition}\label{De:Sequences}
	...
	Then $p$ is said to be a \emph{limit} of $\{x_n\}_{n \in \bbN}$, written $x_n \xrightarrow[n \to \infty]{} p$, if for all neighborhoods $U$ of $p$, there exists $n_0 \in \bbN$ such that for all $n \ge n_0$, we have $x_n \in U$.
\end{definition}
...
\begin{definition}\label{De:ContinuousFromInterval}
	...
	A function $r : [a,b] \to X$ is said to be \emph{continuous} if for all $x_0 \in [a,b]$, for all sequences $\{x_n\}_{n \in \bbN} \subseteq [a,b]$, $x_n \rightarrow x_0$ implies $r(x_n) \rightarrow r(x_0)$.
\end{definition}
