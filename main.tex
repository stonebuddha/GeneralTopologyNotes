...
\begin{proof}
	...
	By \lemref{InfiniteSubsetOfCompactHasLimit}, we know that there exists $x_0 \in X$ such that $x_0 \in E'$.
	By \propref{OrigPlusLimitIsClosure} we know that $x_0 \in \overline{E}$.
	Then by \lemref{FirstCountLimitOfSeqIffClosure} we know that there exists $\{y_m\}_{m \in \bbN} \subseteq E$ such that $y_m \rightarrow x_0$.
...
\end{proposition}
\begin{proof}
	The ${\implies}$ direction is trivial: Because $X$ is a metric space, $X$ is first countable, and then by \propref{CompactFstCountThenSeqCompact} we conclude that $X$ is sequentially compact.
	...
	By \lemref{MetricSeqCompactIsTotallyBounded}, $X$ is totally bounded and $a	\coloneqq \mathrm{diam}(X) < +\infty$.
...
\end{proposition}
\begin{proof}
	By \lemref{MetricSeqCompactIsTotallyBounded}, $X$ is totally bounded.
	...
	By \propref{MetricSeparableImplySndAxCount}, we know that the metric space $X$ is second countable.
\end{proof}
...
\begin{remark}\label{Rem:AnotherProofMetricSeqCompactImplyCompact}
	Here we give another proof of \propref{MetricCompactIffSeqCompact}.
	Suppose $(X,d)$ is a sequentially compact metric space.
	Let $X = \bigcup_{\alpha \in \Lambda} U_\alpha$ where $U_\alpha$'s are open sets.
	By \propref{MetricSeqCompactImplySeparable}, $X$ is second countable.
	By \propref{MetricSndCountImplyLindelof}, $X$ is Lindel{\"o}f, so there exists a countable subcover $X = \bigcup_{n=1}^\infty U_n$.
...
\begin{proof}
	...
	Because $X$ is compact, by \propref{SeveralPropertiesOfCompactness}(1) we know that $C$ is also compact.
	Thus by \propref{ContFuncPreserveCompact}, $f(C) = g^{-1}(C)$ is compact in $Y$.
	Because $Y$ is $T_2$, by \propref{SeveralPropertiesOfCompactness}(2) we know that $g^{-1}(C)$ is closed in $Y$.
	Then by \propref{SomeEquivResultsAboutContFunc}(3) we know that $g$ is continuous.
\end{proof}
...
\begin{proof}
	\begin{itemize}
		\item First proof: Because $X$ is compact and $f$ is continuous, by \propref{ContFuncPreserveCompact}, we know $f(X)$ is compact in $\bbR$.
		Then by \theoref{HeineBorelProperty} we know that $f(X)$ is closed and bounded.
...
\begin{proof}
	...
	The projection maps $\pi_X : X \times Y \to X$ and $\pi_Y : X \times Y \to Y$ are indeed continuous, thus by \propref{ContFuncPreserveCompact} we conclude that $X$ and $Y$ are compact.
...
\begin{proof}\
	\begin{enumerate}
		...
		\]
		and $X \setminus U_\beta$ is closed and compact, so by \propref{SeveralPropertiesOfCompactness}(1) we know that $X \setminus (\bigcup_{\alpha \in \Lambda} U_\alpha)$ is also compact.
		...
		\begin{itemize}
			...
			By \propref{SeveralPropertiesOfCompactness}(1) we know that $\overline{U \cap X}$ is compact.
...
\begin{proof}\
	\begin{itemize}
		...
		Because projection maps $\pi_\alpha$ are continuous, by \propref{ContImplySeqCont}, we know that they are also sequentially continuous, thus
		\[
...
\end{lemma}
\begin{proof}
	The $\implies$ direction is easy since composition of continuous functions is continuous (\propref{ContCompIsCont}).
...
\begin{proof}\
	\begin{itemize}
		\item WTS $\implies$: Trivial by \propref{ContFuncPreserveCompact} and the fact that projection maps $\pi_\alpha$ are continuous.
		...
		By \lemref{CompactIffCompactWRTSubbase}, we conclude that $\prod_{\alpha \in \Lambda} X_\alpha$ is compact.
	\end{itemize}
\end{proof}
...
\end{proposition}
\begin{proof}
	The $\implies$ direction is straightforward by \propref{ContPreserveConn} and the fact that projection maps $\pi_X$ and $\pi_Y$ are continuous.
	...
	Since $(X \times \set{y_1}) \cap (\set{x_2} \times Y) = \{ (x_2,y_1) \} \neq \emptyset$, by \propref{UnionConnStillConn}, we know that $(X \times \set{y_1}) \cup (\set{x_2} \times Y)$ is connected.
...
\end{theorem}
\begin{proof}
	The $\implies$ direction is trivial by \propref{ContPreserveConn} and the fact that the projection maps $\pi_\beta$ are continuous for all $\beta \in \Lambda$.
	...
	Next, we claim that $E$ is connected, and then by \propref{ClosedConnComponentStillClosed} we conclude that $\overline{E}$ is connected.
	...
	By induction with \propref{ProdConnIffBothConn} we know that $X_{\alpha_1} \times \cdots \times X_{\alpha_n}$ is connected, thus by \propref{ContPreserveConn} we know that $F_\calI$ is connected.
\end{proof}
...
\begin{proof}\
	\begin{itemize}
		...
		By \remref{T2AndCompletelyRegularImplyCompletelyT2}, we know that $X$ is completely Hausdorff, so there exists $f \in C_b(X)$ such that $f(x) \neq f(y)$.
		Then $e(x)(f) = f(x) \neq f(y) = e(y)(f)$ and hence $e(x) \neq e(y)$.
		
		\item Then we want to show $e$ is a homeomorphism between $X$ and $\beta$.
		\begin{itemize}
			\item WTS $e$ is continuous: It suffices to show that for all $f \in C_b(X)$, $\pi_f \circ e$ is continuous, by \lemref{ProdTopContFuncIffContEveryAxis}.
		...
		\item Finally, by \theoref{Tikhonov} we know that $Y_0$ is compact, $Y = \overline{\beta} \subseteq Y_0$ is a closed subset, then by \propref{SeveralPropertiesOfCompactness}(1) we know that $Y$ is also compact.
...
\begin{remark}\label{Rem:StoneCechNecessary}
	\theoref{StoneCechCompactification} requires $X$ to be Hausdorff and completely regular.
	...
	By \theoref{LocCompT2StrongerCutOff}, there exists $f \in C_c(Y_0)$ such that $f|_K \equiv 1$ and $f|_{C} \equiv 0$.
...
\end{theorem}
\begin{proof}[Proof 1]
	By \lemref{LocCompT2ThenNeighborContainCompactClosure}, there exists an open set $V$ such that $\overline{V}$ is compact and $K \subseteq V \subseteq \overline{V} \subseteq U$.
	Let $X_\infty$ be the one-point compactification of $X$ by \theoref{AlexandroffCompactification}.
	...
	Then by \theoref{Urysohn} we know that there exists a continuous function $f : X_\infty \to [0,1]$ such that $f|_K \equiv 1$ and $f |_{X_\infty \setminus V } \equiv 0$.
...
\begin{proof}
	...
	f(x) \coloneqq \begin{dcases}
 	...
 	\end{dcases}
	\]
	then $f$ is indeed a continuous function by \propref{SomeEquivResultsAboutContFunc}(3).
	By assumption, there exists a continuous extension $g : X \to [0,1]$ of $f$.
	Then $g|_{C_1} = f|_{C_1} \equiv 1$, $g|_{C_2} = f_{C_2} \equiv 0$.
	By \theoref{Urysohn} we know that $X$ is normal.
