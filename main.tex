\documentclass[screen,single]{techreport}
...
\begin{theorem}[Cauchy's lemma]\label{The:CauchyLemma}
...
\begin{proof}
  ...
  Set $\varphi_\alpha \coloneqq \nicefrac{f_\alpha}{\sum_{\beta \in \Lambda} f_\beta}$ to be continuous functions.
...
\begin{theorem}[Urysohn's metrization theorem]\label{The:UrysohnMetrization}
...
\begin{proof}\
  \begin{itemize}
    ...
    Let $n_\epsilon \in \bbN$ be such that $\sum_{n = n_\epsilon+1}^\infty \nicefrac{1}{n^2} < \nicefrac{\epsilon^2}{8}$.
...
\begin{theorem}[Nagata–Smirnov metrization theorem]\label{The:NagataSmirnov}
...
\begin{proof}
  ...
  Fix $\epsilon > 0$.
  Then there exists $n_o \in \bbN$ such that for all $n,m \ge n_o$, we have $|a_n-a_m| < \nicefrac{\epsilon}{2}$.
  Pick $k_o \gg 1$ such that $n_{k_o} \ge n_o$ and for all $k \ge k_o$, $|a_{n_k} - a| < \nicefrac{\epsilon}{2}$.
...
\begin{proof}
  ...
  Since $\{x_n\}_{n \in \bbN}$ is Cauchy, there exists $n_\epsilon \in \bbN$ such that for all $n,m \ge n_\epsilon$, we have $d_X(x_n,x_m) < \nicefrac{\epsilon}{2}$.
  Thus
  \[
  ...
  Fix $\epsilon > 0$.
  Let $n_o \in \bbN$ be such that for all $n,m \ge n_o$, we have $d_X(x_n,x_m) < \nicefrac{\epsilon}{2}$.
...
\begin{proof}\
  \begin{itemize}
    ...
    Fix $\epsilon > 0$.
    Find $n_o \in \bbN$ such that for all $n,m \ge n_o$, we have $d_\infty(f_n,f_m) < \nicefrac{\epsilon}{2}$.
    Then for all $x \in X$, choose $n_1 = n_1(x) > n_o$ such that $d_Y(f_{n_1}(x),f(x)) < \nicefrac{\epsilon}{2}$.
...
\begin{theorem}[Baire category theorem]\label{The:BaireCategory}
...
\begin{proof}
  ...
  Inductively, consider $n \in \bbN$, $x_n \in X$, and $0 < r_n < \nicefrac{1}{n}$.
  Since $x_n \in X = \overline{U_{n+1}}$, we have $B(x_n,r_n) \cap U_{n+1} \neq \emptyset$, thus we find $x_{n+1} \in B(x_n,r_n) \cap U_{n+1}$ and $0 < r_{n+1} < \nicefrac{1}{n+1}$ such that $B(x_{n+1},2r_{n+1}) \subseteq B(x_n,r_n) \cap U_{n+1}$.
  ...
  Fix $\epsilon > 0$. Pick $k \gg 1$ such that $\nicefrac{2}{k} < \epsilon$.
  Then for all $n,m \ge k$, we have $x_n,x_m \in B(x_k,r_k)$ hence $d(x_n,x_m) \le d(x_n,x_k)+d(x_k,d_m) < 2r_k < \nicefrac{2}{k} < \epsilon$.
  ...
  Fix $\ell \in \bbN$.
  For all $n \ge \max\{ \ell, \ceil{\nicefrac{1}{r_\ell}}\}$, we know that $x_n \in B(x_\ell,r_\ell)$, then
  \[
...
\begin{proof}
  Suppose not.
  Then there exists $\epsilon_o > 0$ and we can find $\{x_n\}_{n \in \bbN}$, $\{x_n'\}_{n\in \bbN}$ such that $d_X(x_n,x_n') < \nicefrac{1}{n} \rightarrow 0$ and $d_Y(f(x_n),f(x_n')) \ge \epsilon_o$ for all $n\in \bbN$.
...
\begin{proof}\
  ...
  Fix $\bar{x}, \bar{x}' \in \overline{E}$ such that $d_X(\bar{x},\bar{x}') < \nicefrac{\delta}{2}$.
...
\begin{theorem}[Banach's construction principle]\label{The:BanachConstrPrinc}
  Let $(X,d)$ be a complete metric space.
...
\begin{proof}
  ...
  \]
  Since $L < 1$, we conclude that for any $\epsilon > 0$ we can find $n_o \gg 1$ such that $d(x_0,x_1) \cdot\nicefrac{L^n}{(1-L)} < \epsilon$.
...
\begin{example}\label{Exa:SolvingIVP}
  ...
  \underline{Claim}. $F$ is a contraction if $T < \nicefrac{1}{L}$.
  \begin{align*}
  ...
  Therefore by \cref{The:BanachConstrPrinc}, there exists a unique $u \in C([0,\nicefrac{1}{L}])$ such that $F(u) = u$, i.e., $u(t) = u_0 + \int_0^t f(s,u(s)) ds$.
...
\begin{theorem}[Brouwer's fixed point theorem]\label{The:BrouwerFixedPoint}
...
\begin{theorem}[Ascoli-Arzel{\`a} theorem]\label{The:AscoliArzela}
...
\begin{proof}
  ...
  Fix $\epsilon > 0$. For each $x \in K$, by equicontinuity of $\calF$ we can find $\delta_x > 0$ such that for all $y \in X$, we have $d(x,y) < \delta_x$ implies that $\sup_{f \in \calF} |f(x) - f(y)| < \nicefrac{\epsilon}{8}$.
  ...
  Thus there exists $n_\epsilon \in \bbN$ such that for all $i=1,\cdots,M$ and $n,m \ge n_\epsilon$, we have $|f_{n,n}(x_{z_i}) - f_{m,m}(x_{z_i})| < \nicefrac{\epsilon}{2}$.
  ...
  Fix $x_o \in X$ and $\epsilon > 0$.
  By equicontinuity, there exists $\delta_{x_o} > 0$ such that $d(x,x_o) < \delta_{x_o}$ implies that $\sup_{f \in \calF} |f(x)-f(x_o)| < \nicefrac{\epsilon}{3}$.
...
\begin{proof}\
  \begin{itemize}
    ...
    If $\calF$ is not equicontinuous, there exists $\epsilon_o > 0$ such that for all $n \in \bbN$, there exists $x_n,y_n \in X$ and $f_n \in C(X;\bbR)$ such that $d(x_n,y_n) < \nicefrac{1}{n}$ and $|f_n(x_n) - f_n(y_n)| \ge \epsilon_o$.
    ...
    Let $k_1 \gg 1$ be such that for all $k \ge k_1$, we have $\max_{x \in X} |f_{n_k}(x) - f_o(x)| < \nicefrac{\epsilon_o}{3}$.
    ...
    Hence there exists $\delta > 0$ such that $d(x,y) < \delta$ implies $|f_o(x) - f_o(y)| < \nicefrac{\epsilon_o}{3}$.
...
\begin{theorem}[Dini's theorem]\label{The:Dini}
...
\begin{proof}
  ...
  Define $g(x) \coloneqq \nicefrac{|f(x)|}{M} \in [0,1]$.
  Define
  \[
  g_n(x) \coloneqq p_n\p{ \p{\frac{f(x)}{M}}^2 }, n \in \bbN
  \]
  such that $g_n \in \overline{\calF}$ for all $n \in \bbN$.
  By the claim we know that $g_n \rightarrow \sqrt{\p{\nicefrac{f}{M} }^2} = g$ uniformly on $X$.
  Thus $g \in \overline{\calF}$. Therefore $|f| = M \cdot \nicefrac{|f|}{M} = M \cdot g \in \overline{\calF}$ because $\overline{\calF}$ is an algebra.
